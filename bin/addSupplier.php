<?php
	...
	if(isset($_POST['name']) && isset($_POST['address']) && isset($_POST['phone'])){
		...
		$availability="select * From supplier where name='".$name."'";
		...
		{
		$sql="Insert into supplier (name, address, phone, NIC, joinedDate, remarks) values('$name', '$address', '$phone', '$nic', '$joinedDate', '$remarks')";
		...
		if (!mysqli_query($conn, $sql)){
			...
			echo '<script type="text/javascript"> alert("Supplier was successfully registered"); </script>';
			echo "<a href=../addSupplier.php> Back to Supplier Addition</a>";
		}
		}
		else 
		{
